...
public class ReceiveMediatorQuery extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if(sQuery==null){			
			...
			//Step 3.1. Rewrite values of dimensions and unit
		
			for(i=0; i<arrMetaData.size(); i++)
				arrMetaData.get(i).rewriteResult();		
			
			//Step 3.2. Rewrite observed values if they use different units
			Double multi;
			String unit;
			for(i=0; i<arrMetaData.size(); i++){
				for(j=0; j<arrMetaData.get(i).getNumberofComponent(); j++)
					if(arrMetaData.get(i).getComponent(j).getType().contains("Attribute")){
						...
						if(unit.endsWith("tens"))
							multi=10.0;
						else if(unit.endsWith("hunderds"))
							multi=100.0;					
						if(unit.endsWith("thousands"))
							multi=1000.0;
						else if(unit.endsWith("millions"))
							multi=1000000.0;
						else multi=1.0;
						arrMetaData.get(i).rewriteObservedValue(multi);
					}
			}
			
			//Step 3.3. Rewrite values of refPeriod e.g., /date/1960-01-01 => /year/1960
			for(i=0; i<arrMetaData.size(); i++)
				arrMetaData.get(i).rewriteTemporalValue();
			
			
			//Step 4. Integrate achieved results	
			MetaData result = new MetaData(arrMetaData.get(0));
			
			//find the common values in all results
			for(i=1; i<arrMetaData.size(); i++)
				result.integrateData(arrMetaData.get(i));
			
			//filter each result
			for(i=0; i<arrMetaData.size(); i++)
				arrMetaData.get(i).removeExternalComponent(result);
			
			//Step 5. Return result
			String  sResult;		
			
			if(sFormat==null||sFormat.toLowerCase().contains("json")){
				sResult = getJsonFormat(inputMD, result, arrMetaData);
				response.setContentType("application/json");
			}else if(sFormat.toLowerCase().contains("xml")){
				sResult = getXmlFormat(inputMD, result, arrMetaData);				
				response.setContentType("text/xml");
			}else{
				sResult = getHtmlFormat(inputMD, result, arrMetaData);				
	 ...
	 * 
	*******************************************/
    public static String getJsonFormat(MetaData inputMD, MetaData result, ArrayList<MetaData> arrMetaData){
		...
		//add values
		index = arrMetaData.get(0).getIndexOfMeasureComponent();
		n = result.getComponent(index).getValueSize();	
		
		for(i=0; i<n; i++){					
			for(j=0; j<arrMetaData.size(); j++){
				for(m=0; m<arrMetaData.get(j).getComponent(index).getValueSize(); m++)
					if(result.hasSameValue(i, arrMetaData.get(j), m)){
						sResult = sResult +
								"			{\n";
    ...
    public static String getXmlFormat(MetaData inputMD, MetaData result, ArrayList<MetaData> arrMetaData){
    	int i, j, k, index, n, t, m;
    	ArrayList<String> arrVar = new ArrayList<String>();
    	String sResult="";    	
    ...
    public static String getHtmlFormat(MetaData inputMD, MetaData result, ArrayList<MetaData> arrMetaData){
    	int i, j, k, index, n, t, m;
    	ArrayList<String> arrVar = new ArrayList<String>();
