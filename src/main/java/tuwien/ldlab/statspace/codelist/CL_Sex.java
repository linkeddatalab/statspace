...
public class CL_Sex {
	...
	public CL_Sex(){
		...
		arrCL.add(new StringTriple("http://purl.org/linked-data/sdmx/2009/code#sex-T","Total", null));
		arrCL.add(new StringTriple("http://purl.org/linked-data/sdmx/2009/code#sex-F","Female","http://purl.org/linked-data/sdmx/2009/code#sex-T"));
		arrCL.add(new StringTriple("http://purl.org/linked-data/sdmx/2009/code#sex-M","Male","http://purl.org/linked-data/sdmx/2009/code#sex-T"));
		arrCL.add(new StringTriple("http://purl.org/linked-data/sdmx/2009/code#sex-U","Not specified","http://purl.org/linked-data/sdmx/2009/code#sex-T"));
		arrCL.add(new StringTriple("http://purl.org/linked-data/sdmx/2009/code#sex-N","Not applicable","http://purl.org/linked-data/sdmx/2009/code#sex-T"));	
	}
	...
	public String getUpperLevel(int i){return arrCL.get(i).getThirdString();}
	...
	public String identifyReference(String uri, String label){
		...
		if(uri!=null && !uri.isEmpty()){			
			String code="";
			code = extractSexCode(uri);
			if(!code.isEmpty()){		
				for(i=0; i<arrCL.size(); i++)
					if(code.equalsIgnoreCase(extractSexCode(arrCL.get(i).getFirstString())))
						return arrCL.get(i).getFirstString();
			}
		}
	...
	public String extractSexCode(String s){
		int i = s.length()-1;
	...
	public void generateCodeList(){
		...
		Resource dimension  = mOutput.createResource("http://purl.org/linked-data/sdmx/2009/dimension#sex");
		Resource conceptScheme  = mOutput.createResource(sdmx_code + "sex");
		Resource cls  = mOutput.createResource(sdmx_code+"Sex");
    	
		dimension.addProperty(RDF.type, QB.DimensionProperty);
		dimension.addProperty(RDF.type, RDF.Property);
		literal = mOutput.createLiteral("Sex", "en");
		dimension.addProperty(label, literal);
		literal = mOutput.createLiteral("The state of being male or female", "en");
		...
		literal = mOutput.createLiteral("Code list for Sex (SEX) - codelist class", "en");
		cls.addProperty(label, literal);
		literal = mOutput.createLiteral("This  code list provides the gender", "en");
		...
		conceptScheme.addProperty(notation, "CL_SEX");
		literal = mOutput.createLiteral("This  code list provides the gender", "en");
		...
		try {
			out = new FileOutputStream("data/codelist/cl_sex.ttl");
