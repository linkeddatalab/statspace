...
public abstract class AbstractRMLProcessor implements RMLProcessor {
     ...
     * @return
     */
    protected String getIterator(LogicalSource ls) {
    ...
    public Resource processSubjectMap(SesameDataSet dataset, SubjectMap subjectMap, Object node) {       
        ...
        switch (subjectMap.getTermType()) {
            case IRI:
                ...
                break;
            case BLANK_NODE:
                subject = new BNodeImpl(org.apache.commons.lang3.RandomStringUtils.randomAlphanumeric(10));
                break;
            default:
                subject = new URIImpl(value);
        }
        ...
        for (PredicateMap predicateMap : predicateMaps) {
            ...
            for (URI predicate : predicates) {
                ...
                for (ReferencingObjectMap referencingObjectMap : referencingObjectMaps) {
                    ...
                    String source = parentTriplesMap.getLogicalSource().getSource();
                    ...
                    if (joinConditions.isEmpty() & !parentTriplesMap.getLogicalSource().getSource().equals(map.getLogicalSource().getSource())) {
                    ...
                    else if (joinConditions.isEmpty() & parentTriplesMap.getLogicalSource().getSource().equals(map.getLogicalSource().getSource())){
                        performer = new SimpleReferencePerformer(processor, subject, predicate);
                        if((parentTriplesMap.getLogicalSource().getReferenceFormulation().toString().equals("CSV")) || (parentTriplesMap.getLogicalSource().getIterator().equals(map.getLogicalSource().getIterator()))){
                            performer.perform(node, dataset, parentTriplesMap);
                        }
                        else{
                            int end = map.getLogicalSource().getIterator().length();
                            log.info("RML:AbstractRMLProcessor " + parentTriplesMap.getLogicalSource().getIterator().toString());
                            String expression = "";
