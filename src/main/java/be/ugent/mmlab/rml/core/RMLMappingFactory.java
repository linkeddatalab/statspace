...
import be.ugent.mmlab.rml.model.Parameters.Parameter;
...
import be.ugent.mmlab.rml.vocabulary.Vocab.RMLXTerm;
...
public abstract class RMLMappingFactory {
     ...
     */
    public static RMLMapping extractRMLMapping(String fileToRMLFile, Parameters parameters){
    	try{
	        ...
	        //Replace RDF with parameters
	        replaceParameters(r2rmlMappingGraph, parameters);
           ...
           log.debug("[RMLMappingFactory:extractRMLMapping] Number of RML triples with "
	        ...
	        return result;
    	 } catch (IOException | InvalidR2RMLStructureException | InvalidR2RMLSyntaxException | R2RMLDataError | RepositoryException | RDFParseException ex) {
...
//                    + shortcutTriples.size());
    ...
    private static void replaceParameters(SesameDataSet r2rmlMappingGraph, Parameters parameters) {    	
    	URI u =  vf.createURI(Vocab.RMLX_NAMESPACE + RMLXTerm.DEFAULTVALUE);
    	URI v =  vf.createURI(Vocab.RMLX_NAMESPACE + RMLXTerm.VARNAME);
    	URI t =  vf.createURI(Vocab.R2RML_NAMESPACE + R2RMLTerm.CONSTANT);
    	int i;
    	
        List<Statement> triples = r2rmlMappingGraph.tuplePattern(null, u, null);       
//        log.debug("[RMLMappingFactory:replaceParameters] Number of Parameters found "               
//                + u.getLocalName()
//                + " : "
//                + triples.size());
        
        for (Statement triple : triples) {
        	Resource s = (Resource) triple.getObject();
        	List<Statement> varNames = r2rmlMappingGraph.tuplePattern(s, v, null);
        	List<Statement> defaultValues = r2rmlMappingGraph.tuplePattern(s, t, null);
        	String sVarName = varNames.get(0).getObject().toString().replace("\"", "");
        	String sDefValue = defaultValues.get(0).getObject().toString().replace("\"", "");
        	parameters.addParameterDefValue(sVarName, sDefValue);      

        	//remove this triple
        	r2rmlMappingGraph.remove(s, v, varNames.get(0).getObject());
        	r2rmlMappingGraph.remove(s, t, defaultValues.get(0).getObject());
        	r2rmlMappingGraph.remove(triple.getSubject(), u, s);
        	
        }
        
        
       //consider each triple
       triples = r2rmlMappingGraph.tuplePattern(null, null, null);
       boolean bPar;
       for(Statement triple : triples){
        	Resource s = (Resource) triple.getSubject();
        	URI p = triple.getPredicate();
        	String o = triple.getObject().toString().replace("\"", "");        	
        	bPar = false;
        	for(i=0; i<parameters.getSize(); i++){
        		String sVarName = parameters.getParameter(i).getName();       	
        		if(o.contains("{"+sVarName+"}")){
        			bPar = true;
        			if(parameters.getParameter(i).getValue()=="")
        				o = o.replace("{"+sVarName+"}", parameters.getParameter(i).getDefaultValue());
        			else
        				o = o.replace("{"+sVarName+"}", parameters.getParameter(i).getValue());        			
        		}
        	}
        	if(bPar){
        		r2rmlMappingGraph.remove(s, p, triple.getObject());
        		if(o.startsWith("http"))
        			r2rmlMappingGraph.add(s,  p, vf.createURI(o));
        		else
        			r2rmlMappingGraph.add(s,  p, vf.createLiteral(o));
        	}
       }    
...
//                log.info("[RMLMappingFactory] saved Graphs " + savedGraphMaps);
    ...
    private static LogicalSource extractLogicalSource(
		...
		if (statements.isEmpty()) {
			...
			LogicalSource  logicalSource = new StdLogicalSource("null", "null", Vocab.QLTerm.NULL_CLASS);
