...
//		createPrefix();			
	...
	public static void mergeMetaData2(){
		try{			
			Model model = ModelFactory.createDefaultModel();
			InputStream is;
			is = FileManager.get().open("data/metadata/cr.eionet.europa.eu_sparql1.ttl");		
			model.read(is,null,"TTL");	
			is.close();
			is = FileManager.get().open("data/metadata/opendatacommunities.org_sparql.xml1.ttl");		
			model.read(is,null,"TTL");	
			is.close();
			is = FileManager.get().open("data/metadata/unodc.publicdata.eu_sparql1.ttl");		
			model.read(is,null,"TTL");	
			is.close();			
			is = FileManager.get().open("data/metadata/statistics.gov.scot_sparql1.ttl");		
			model.read(is,null,"TTL");	
			is.close();
			is = FileManager.get().open("data/metadata/ogd.ifs.tuwien.ac.at_sparql1.ttl");		
			model.read(is,null,"TTL");	
			is.close();
			is = FileManager.get().open("data/metadata/data.cso.ie_sparql1.ttl");		
			model.read(is,null,"TTL");	
			is.close();
			is = FileManager.get().open("data/metadata/semantic.eea.europa.eu_sparql1.ttl");		
			model.read(is,null,"TTL");	
			is.close();
			is = FileManager.get().open("data/metadata/data.europa.eu_euodp_sparqlep1.ttl");		
			model.read(is,null,"TTL");	
			is.close();		
			
			OutputStream out = new FileOutputStream( new File( "data/metadata/statspace1.ttl" ));
		    model.write( out, "Turtle", null );
