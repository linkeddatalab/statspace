...
import at.tuwien.ldlab.statspace.codelist.CL_Subject;
...
//						System.out.println(line);					
	...
	public static void removeDuplicateIndicators(){
		try {
			BufferedReader br_in = new BufferedReader(new InputStreamReader(
    			    new FileInputStream("data/indicators2_all.csv"), "UTF-8"));
			
			BufferedWriter br_out = new BufferedWriter(new OutputStreamWriter(
    			    new FileOutputStream("data/indicators2.csv"), "UTF-8"));  		
	...
	public static void checkNewIndicators(){
...
//			for(i=0; i<=100; i++){			
	...
	public static void removeRDFFiles(){		
		int i, k;
		String sIndicator;
		try{
			for(i=0; i<indicators.getSize(); i++){			
				...
				if(!file.exists()){
					System.out.println("Deleting: "+ i + "\t"+ sIndicator);
					file.delete();					
				}
			}
	...
	public static void checkRedundantFiles(){		
		int i, k;
		String sIndicator;
		try{
		   File directory = new File("data/wb");		
		   String folders[] = directory.list();	 
       	   for (String name : folders) {
       		   for(i=0; i<indicators.getSize(); i++){
       			   ...
       			   k = sIndicator.indexOf("indicator");
       			   sIndicator = sIndicator.substring(k+10);
       			   if(sIndicator.equals(name))
       				   break;
       		   }
       		   if(i==indicators.getSize())
