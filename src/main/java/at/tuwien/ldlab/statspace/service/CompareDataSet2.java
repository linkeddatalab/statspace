...
public class CompareDataSet2  extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static Log log = LogFactory.getLog(CompareDataSet2.class);
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if(sMDUri1!=null && !sMDUri1.isEmpty() && sMDUri2!=null && !sMDUri2.isEmpty()){
        	...
        	if(bValid==false){
				...
				for(i=0; i<inputs.size(); i++){		
					delay(1);
	           		inputs.get(i).rewriteQuery(sVarObs, sWebApp, sSeparator,false, bUseCache);           		
				}
...
//				}
				...
				if(inputs.get(0).getComponent(0).getValueSize()==0 || inputs.get(1).getComponent(0).getValueSize()==0){
		    		...
		    		RequestDispatcher view = request.getRequestDispatcher("/explorer/compare.jsp");
